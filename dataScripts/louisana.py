...
water_label = water_hist.select('occurrence').gt(80).rename('water_label').toFloat()

final_stack = image.addBands(water_label).toFloat()

# Export to Google Drive
task = ee.batch.Export.image.toDrive(
    image=final_stack,
    description='Louisiana_Wetland_2024',
    folder='Nature_Restoration_Louisiana',
    scale=10,
    region=roi_la,
    fileFormat='GeoTIFF',
    maxPixels=1e10,
